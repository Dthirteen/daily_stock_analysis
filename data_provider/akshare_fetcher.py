...
class AkshareFetcher(BaseFetcher):
    ...
    def get_sector_rankings(self, n: int = 5) -> Optional[Tuple[List[Dict], List[Dict]]]:
        ...
        def _get_rank_top_n(df: pd.DataFrame, change_col: str, industry_name: str, n: int) -> Tuple[list, list]:
            ...
            top_sectors = [
                _build_sector_dict(row, industry_name, change_col)
        ...
        def _build_sector_dict(self, row: pd.Series, name_col: str, change_col: str) -> Dict:
            """从板块排行 DataFrame 行构建字典，尽可能包含完整字段。"""
            d = {
                'name': row.get(name_col, ''),
                'change_pct': float(row.get(change_col, 0)),
            }
            # 尝试提取可选字段（不同接口列名可能不同）
            amount_candidates = ('成交额', 'amount', '总金额', 'main_net_amount', 'net_amount')
            for col in amount_candidates:
                if col in row.index and pd.notna(row.get(col)):
                    try:
                        d['amount'] = float(row[col])
                        break
                    except (ValueError, TypeError):
                        continue

            vol_candidates = ('成交量', 'volume', '总成交量')
            for col in vol_candidates:
                if col in row.index and pd.notna(row.get(col)):
                    try:
                        d['volume'] = float(row[col])
                        break
                    except (ValueError, TypeError):
                        continue

            stock_count_candidates = ('上涨家数', 'stock_count', '公司数', '成分股数量')
            for col in stock_count_candidates:
                if col in row.index and pd.notna(row.get(col)):
                    try:
                        d['stock_count'] = int(float(row[col]))
                        break
                    except (ValueError, TypeError):
                        continue

            up_count_candidates = ('上涨家数', 'up_count')
            for col in up_count_candidates:
                if col in row.index and pd.notna(row.get(col)):
                    try:
                        d['up_count'] = int(float(row[col]))
                        break
                    except (ValueError, TypeError):
                        continue
